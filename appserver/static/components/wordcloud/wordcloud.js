...
define(function(require, module) {
	...
	//var cloud=require("./contrib/cloud");
...
tags = {};
	...
	val=t1.productCount;
        ...
        discard.test(t) || (t = t.replace(punctuation, ""), stopWords.test(t.toLowerCase()) || (t = t.substr(0, maxLength), e[t.toLowerCase()] = t, tags[t = t.toLowerCase()] = (tags[t] || 0) +val))
...
function generate() {
    layout.font("Impact").spiral("archimedean"), fontSize = d3.scale["log"]().range([12, 50]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], layout.stop().words(tags.slice(0, max = Math.min(tags.length, +250))).start()
}
...
function draw(t, e) {
    ...
    n.transition().duration(1e3).attr("transform", function(t) {
        ...
        return fill(t.text.key.toLowerCase())
    }).text(function(t) {
...
parseText(data), d3.select("#random-palette").on("click", function() {
        paletteJSON("http://www.colourlovers.com/api/palettes/random", {}, function(t) {
            fill.range(t[0].colors), vis.selectAll("text").data(words).style("fill", function(t) {
    ...
    function() {
        function t() {
            c = +5, u = Math.max(-90, Math.min(90, +d3.select("#angle-from").property("value"))), i = Math.max(-90, Math.min(90, +d3.select("#angle-to").property("value"))), e()
        }
        ...
        function e() {
            ...
            }),  d3.select("#angle-from").property("value", u), d3.select("#angle-to").property("value", i)
        }
        ...
        d3.selectAll("#angle-from, #angle-to").on("change", t).on("mouseup", t), t()
    }();
        }
    });
	return WordCloud;
